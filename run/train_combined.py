...
def main(cfg: DictConfig):  # type: ignore
    ...
    save_path = os.path.join(cfg.dir.output_dir, "train", cfg.exp_name, "single", "/".join(checkpoint_cb.best_model_path.split('/')[:-2]), "best_model_weights.pth")
    cfg_save_path = os.path.join(cfg.dir.output_dir, "train", cfg.exp_name, "single", "/".join(checkpoint_cb.best_model_path.split('/')[:-2]), "best_cfg.pkl")
